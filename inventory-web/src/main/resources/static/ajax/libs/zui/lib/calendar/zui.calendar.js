                    ...
                    for(i = 0; i <= dayEvents.maxPos; ++i) {
                        ...
                        if (options.eventCreator) {
                            $event = options.eventCreator(e, $cell, that);
                        		...
                        		// chenpei 20190806  修改原有方法  新增以小圆圈示例  <span class="cp-circle"></span> 
                            $event = $('<div data-id="' + e.id + '" class="event" title="' + e.desc + '"><span class="cp-circle"></span><span class="time">' + e.start.format('hh:mm') + '</span> <span class="title">' + e.title + '</span></div>');
                            ...
                            if(cal) {
                                if(cal.presetColor) {
                                    $event.addClass('color-' + cal.color);
                                		...
                                		// chenpei 20190806 配合小圆圈修改
	                                	$event.find('.cp-circle').css({
