/**
 * 首页方法封装处理 Copyright (c) 2019 ruoyi
 */
layer.config({
...
$(function() {
	...
	/*
	 * $(".menuItem").each(function(index) { if (!$(this).attr('data-index')) {
	...
	function menuItem() {
		...
		// 选项卡菜单不存在
		if (flag) {
			var str = '<a href="javascript:;" class="active menuTab" data-id="iframe' + dataIndex + '">' + menuName
	...
	function closeOtherTabs() {
		$('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
			$('.RuoYi_iframe[name="' + $(this).data('id') + '"]').remove();
			$(this).remove();
		});
	...
	function refreshTab() {
		...
		var target = $('.RuoYi_iframe[name="' + currentId + '"]');
	...
	$('.tabCloseAll').on('click', function() {
		...
		$('.page-tabs-content').children("[data-id]:first").each(function() {
			$('.RuoYi_iframe[name="' + $(this).data('id') + '"]').show();
